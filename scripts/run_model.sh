...
python prepare_data/preparing_faces_parallel.py --mode align --root_path /workspace/data/Camera

# we for some reason need to do both?
python prepare_data/preparing_faces_parallel.py --mode crop --root_path /workspace/data/Camera --random_shift 0.05
...
--raw_root /workspace/data/Camera \
--aligned_root /workspace/data/Camera_aligned \
--cropped_root /workspace/data/Camera_croped \
--output_root /workspace/data/landmarks_transforms/

#Run the the model with their edit image script
time LANG=en_US.utf8 python inversion/scripts/inference_editing_random.py \
--output_path /workspace/data/test \
--checkpoint_path pretrained_models/restyle_pSp_ffhq.pt \
--data_path /workspace/data/Camera_aligned \
--test_batch_size 4 \
--test_workers 4 \
--n_iters_per_batch 3 \
--edit_directions "[age,pose,smiling]" \
--factor_ranges "[(-5_5),(-5_5),(-5_5)]" \
--landmarks_transforms_path /workspace/data/landmarks_transforms/landmarks_transforms.npy \
