...
def run(test_opts: TestOptions):
    ...
    for input_batch in tqdm(dataloader):
        ...
        with torch.no_grad():
            ...
            tic = time.time()
            result_batch, factors_batch = edit_batch(inputs=input_batch.cuda().float(),
                                      net=net,
                                      avg_image=avg_image,
        ...
        print(f"Processed batch of size {input_batch.shape[0]}")
        for i in range(input_batch.shape[0]):
            ...
            for edit_name, edit_res in results.items():
                for idx, result in enumerate(edit_res): # Save each edited image separately
                    metadata = PngInfo()
                    ...
                    result.save(edit_save_dir / f"{im_path.stem}_{idx}.png", pnginfo=metadata)
                    os.chown(edit_save_dir / f"{im_path.stem}_{idx}.png", 1000, 1000) # change ownership to user "bunn" (uid 1000)
            global_i += 1
...
def edit_batch(inputs: torch.tensor, net, avg_image: torch.tensor, latent_editor: FaceEditor, opts: TestOptions,
    ...
    for edit_direction, factor_range in zip(opts.edit_directions, opts.factor_ranges):
        ...
        edit_images, _, factors_used = edit_res
    ...
    return results, results_factors
